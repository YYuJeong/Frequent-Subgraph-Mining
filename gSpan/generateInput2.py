...
f=open('./gSpan/graphdata/output.txt','w', encoding='utf-8')
stdout=sys.stdout
sys.stdout=f
    ...
    for n in DG1.nodes(data=True):
        ...
        if 'Data' in n[1]['labels']:
            ...
            if(n[1]['properties'].get('file_path')):
                UnLabeled = 'data'+ n[1]['properties']['name']+ n[1]['properties']['file_path']
            else:
                UnLabeled = 'data'+ n[1]['properties']['name']+ n[1]['properties']['value']

            NodeLabel[0].append(UnLabeled)        
